...
class Parser {
  ...
  parse () {
    ...
    .then(results => {
      ...
      return fs.writeFile(path.join(this.options.output, 'list.md'), md, this.options.encoding).then(() => {
        results.push({
          path: '/',
          name: 'list.md',
          file: path.join(this.options.output, 'list.md')
        })
    ...
    .then(result => {
      ...
      const hierarchy = {}
      // filenames: { filename.md: path }

      Object.keys(filenames).forEach(fn => {
        let path = filenames[fn]
      ...
      let md = '# Signal K - Key reference\n\n'
      let level = 0

      md = this.renderNestedList(md, level, hierarchy)
  ...
  renderNestedList (md, level, object, rootKey) {
    const keys = Object.keys(object)
    
    if (typeof object['@file'] !== 'undefined') {
      const fn = object['@file']
      md += this.renderTabs(level)
      md += `* [${rootKey}](${fn})\n`
    }
    ...
    if (keys.length > 1) {
      keys.forEach(key => {
        level += 1
        md = this.renderNestedList(md, level, object[key], key)
      })
    }
    ...
    if (title === '/') {
      html += `<script>
        $(document).ready(function () {
          var exclude = ['/_attr', '/$source', '/timestamp', '/meta', '/pgn', '/sentence', '/value']

          $('li').each(function () {
            var path = $(this).find('a').text()
            var excl = false

            exclude.forEach(function (mixed) {
              if (typeof mixed === 'string' && path.indexOf(mixed) !== -1) {
                excl = true
              }

              if (typeof mixed === 'object' && mixed !== null && typeof mixed.test === 'function' && mixed.test(path) === true) {
                excl = true
              }
            })
          ...
          $('input[type="search"]').on('keyup', keyup)
          $('li[data-hidden]').hide()
        })
      </script>`

      html += `<section class="container">
        <input type="search" placeholder="Filter" style="border-radius: 5px; border: 1px solid #ccc; display: block; width: 100%; margin: 2em 0; padding: .25em .5em;">
      </section>`
    }

    html += `<section class="container" id="data">${str}</section>`
    html += '</body></html>'
